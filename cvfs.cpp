...
void man(char *name)
{
    ...
    {
        printf("Description : Used to display the information of files.\n");
...
int ReadFile(int fd, char *arr, int iSize)
{
    ...
    {
        strncpy(arr,(UFDTArr[fd].ptrfiletable -> ptrinode -> Buffer) + (UFDTArr[fd].ptrfiletable -> readoffset) + (UFDTArr[fd].ptrfiletable -> readoffset),iSize);
...
void CloseAllFile()
{
    ...
    while(i < 50)
    {
        if(UFDTArr[i].ptrfiletable != NULL)
        {
            ...
            if(UFDTArr[i].ptrfiletable -> writeoffset = 0);
            (UFDTArr[i].ptrfiletable -> ptrinode -> ReferenceCount)--;
            break;
        }
    }
